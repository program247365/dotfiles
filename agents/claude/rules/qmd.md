...
- Do not run `qmd collection add`, `qmd embed`, or `qmd update` automatically — write out commands for the user to run. `search update qmd` is the sanctioned wrapper for update + embed (see search rule).
