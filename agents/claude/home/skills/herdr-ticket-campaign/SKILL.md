...
Branch name uses a prefix from the repo reference file's blessed list — the campaign monitor matches on prefix, so an off-list branch silently falls out of PR monitoring. Agent `<name>`: a short kebab slug of the feature (`delete-room`, `thread-actions`), not the ticket id; must be unique among live agents (`herdr agent list` first — names from prior campaigns can linger until their panes close).
